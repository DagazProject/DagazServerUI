...
export class SessionComponent implements OnInit {
  ...
  public getStyles(game: number, variant: number) {
    const r = this.styles.filter((it: Style) => { return it.variant_id == variant; });
    if (r.length > 0) return r;
    return this.styles.filter((it: Style) => { return it.game_id == game; });
  }

  public isStyled(game: number, variant: number) {
    const s = this.getStyles(game, variant);
    return s.length > 1;
  }
  ...
  private initStyle(session) {
    const styles = this.getStyles(session.game_id, session.variant_id);
